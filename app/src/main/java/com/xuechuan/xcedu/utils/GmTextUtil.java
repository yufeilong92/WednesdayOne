...
public class GmTextUtil {
     ...
     * @return
     */
    public ArrayList<SubmiteExamVo> getUserDoLog(ArrayList<DoBankSqliteVo> list) {
        ArrayList<SubmiteExamVo> vos = new ArrayList<>();
    ...
    //重组数据
    public void doDataListCaseEvent(ArrayList<CaseCardVo> mCardLists, ArrayList<QuestionCaseVo> lists) {
        CaseCardVo caseCardVo = new CaseCardVo();
        //题干
        ArrayList<CaseCardVo> vos = new ArrayList<>();
        if (lists == null || lists.isEmpty()) return;
        for (int i = 0; i < lists.size(); i++) {
            ...
            if (vo.getQuestiontype() == 4 || vo.getQuestiontype() == 5) {
                ...
                if (integers == null || integers.isEmpty()) {
                    ...
                    caseCardVo.setMaintype(1);
                    caseCardVo.setCasevo(vo);
                    ArrayList<CaseCardVo> integerList = getIntegerCaseList(integers, lists);
                    caseCardVo.setList(integerList);
                    if (!mCardLists.contains(caseCardVo))
                        mCardLists.add(caseCardVo);
                    //删除防止出现死循环
     ...
     */
    //获取重复数据
    public ArrayList<CaseCardVo> getIntegerCaseList(ArrayList<Integer> integers, ArrayList<QuestionCaseVo> datas) {
        if (integers == null || integers.isEmpty()) return null;
        if (datas == null || datas.isEmpty()) return null;
        ArrayList<CaseCardVo> sqliteVos = new ArrayList<>();
        for (int i = 0; i < datas.size(); i++) {
    ...
    //删除重复数据
    public ArrayList<QuestionCaseVo> getDeleteIntegerCaseList(ArrayList<Integer> integers, ArrayList<QuestionCaseVo> datas) {
        if (integers == null || integers.isEmpty()) return datas;
        if (datas == null || datas.isEmpty()) return null;
        for (int i = 0; i < datas.size(); i++) {
            QuestionCaseVo vo = datas.get(i);
    ...
    //重组数据(只有简答解析)
    public void doGalleryListCaseEvent(ArrayList<CaseCardVo> mCardLists, ArrayList<QuestionCaseVo> lists) {
        CaseCardVo caseCardVo = new CaseCardVo();
        if (lists == null || lists.isEmpty()) return;
        for (int i = 0; i < lists.size(); i++) {
            QuestionCaseVo vo = lists.get(i);
            if (vo.getQuestiontype() == 4 || vo.getQuestiontype() == 5) {
                //获取题干小题
                ArrayList<Integer> integers = groupCaseExamData(vo.getQuestion_id(), lists);
                if (integers != null && !integers.isEmpty()) {
                    caseCardVo.setType(2);
                    caseCardVo.setMaintype(1);
                    caseCardVo.setCasevo(vo);
     ...
     * @return
     */
    public ArrayList<Integer> groupCaseData(int id, ArrayList<QuestionCaseVo> mData) {
        ArrayList<Integer> ints = new ArrayList<>();
        for (int i = 0; i < mData.size(); i++) {
     ...
     * @return
     */
    public ArrayList<Integer> groupCaseExamData(int id, ArrayList<QuestionCaseVo> mData) {
        ArrayList<Integer> ints = new ArrayList<>();
        for (int i = 0; i < mData.size(); i++) {
            QuestionCaseVo vo = mData.get(i);
            if (vo.getParent_id() == id) {
