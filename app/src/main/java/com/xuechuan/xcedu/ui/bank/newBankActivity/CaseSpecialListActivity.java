...
*/
    ...
    public ArrayList<CaseCardVo> getLists() {
        ...
        ArrayList<QuestionCaseVo> list = mQuestionSqliteHelp.queryQuestionVoWithTagCase(listTags);
    ...
    public void doRightGo() {
        if (mReaderViewPagerone == null) return;
        int currentItem = mReaderViewPagerone.getCurrentItem();
        currentItem = currentItem + 1;
        if (currentItem < 0) {
            T.showToast(mContext, "已经是最后一题");
            currentItem = 0;
        }
        mReaderViewPagerone.setCurrentItem(currentItem, true);
...
//                            Log.e("===", "执行");
    ...
    private void doDoText() {
        ...
        if (mQuesiton_id != -1 && prePosition1 != -1) {
